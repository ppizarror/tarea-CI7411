function [f, hv_ns, hv_ew] = hv_gen(few, fns, fz, filtro)
...
% Calcula H/V
hv_ns = zeros(1, t_len_h);
hv_ew = zeros(1, t_len_h);
hv = zeros(1, t_len_h);
for i = 1:t_len_h
    hv_ns(i) = fft_ns(i) / fft_z(i);
    hv_ew(i) = fft_ew(i) / fft_z(i);
...
hold on;
plot(freq_h, hv_ew);
%semilogx(freq_h, hv_ns);
%semilogx(freq_h, hv_ew);
plot(freq_h, hv);
hold on;
% xlim([0.1, 10]);
grid on;
